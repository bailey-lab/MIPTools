...
sub geneOperation {
	...
	#generate gene anno
	my $sc;
	$sc = "annotate_pf_variation.pl -geneanno -buildver $buildver -dbtype $protocol -outfile $tempfile.$protocol -exonsort $queryfile $dbloc";
...
sub regionOperation {
	...
	my ($userfile, $header);
	my $sc = "annotate_pf_variation.pl -regionanno -dbtype $dbtype -buildver $buildver -outfile $tempfile $queryfile $dbloc";
	$argument and $sc .= " $argument";
...
sub filterOperation {
	my ($dbtype, $dbtype1, $argument) = @_;
	my ($userfile, $header);
	my $sc = "annotate_pf_variation.pl -filter -dbtype $dbtype -buildver $buildver -outfile $tempfile $queryfile $dbloc";
	$argument and $sc .= " $argument";
	if ($thread) {
		$sc .= " -thread $thread";
	}
