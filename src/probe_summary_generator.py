...
def get_probe_call_info(output_file=None,
                        probe_sets_file=None, 
                        call_info_file=None):
    if probe_sets_file is None:
        probe_sets_file = "/opt/project_resources/mip_ids/mip_arms.txt"
    if call_info_file is None:
        call_info_file = "/opt/project_resources/mip_ids/call_info.json"
    arms_df = pd.read_csv(probe_sets_file, sep='\t')
    used_probes = set(arms_df['mip_family'].to_list())
